...
@pytest.mark.parametrize(
    "source",
    [SourceCodes.CD, SourceCodes.FM, SourceCodes.DAB, SourceCodes.NET],
)
async def test_set_display_info_type_on_every_source(source):
    client = MagicMock(spec=Client)
    state = make_state(client, 1, ApiModel.APIHDA_SERIES)
    state._state[CURRENT_SOURCE.cc] = source.to_bytes(state.api_model, 1)
